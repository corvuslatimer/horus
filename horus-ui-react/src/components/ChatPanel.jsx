...
export default function ChatPanel() {
  ...
  useEffect(() => {
    fetch('http://localhost:8787/api/chat').then(r=>r.json()).then(j=>setMessages((j.messages||[]).slice(-20))).catch(()=>{})
  }, [])
  ...
  const send = async (e) => {
    ...
    try {
      const r = await fetch('http://localhost:8787/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg})})
  ...
  return (
    <div style={{flex:2,display:'flex',flexDirection:'column',borderTop:'1px solid #1a1a1a',background:'#0b0b0b',minHeight:170}}>
      ...
      <div ref={logRef} style={{flex:1,overflowY:'auto',padding:'8px 10px',display:'flex',flexDirection:'column',gap:6}}>
        {messages.map((m,i) => (
          <div key={i} style={{
            fontSize:11,lineHeight:1.35,padding:'6px 8px',borderRadius:6,maxWidth:'95%',
            alignSelf:m.role==='user'?'flex-end':'flex-start',
            background:m.role==='user'?'#1b2a3d':'#1b1b1b',
            color:m.role==='user'?'#dbeafe':'#ddd',
